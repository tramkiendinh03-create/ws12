    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[480px]">
      ...
      <MetalContainer className="lg:col-span-1 h-full flex flex-col" glowColor="red" title="反派列表" noPadding>
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="relative shrink-0">
                    ...
                    <div
                      className={`absolute -top-1 -right-1 w-5 h-5 flex items-center justify-center rounded-full bg-black border text-[10px] font-bold ${getThreatColor(villain.威胁度)} border-current`}
                    >
                      {villain.威胁度}
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4
                      className={`text-sm font-bold truncate mb-1 ${isSelected ? 'text-white' : 'text-gray-400 group-hover:text-gray-200'}`}
                    >
                      {villain.姓名}
                    </h4>
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-1.5 py-[1px] rounded text-[10px] font-bold border ${isSelected ? 'bg-white/20 border-white/30 text-white' : 'bg-white/5 border-white/10 text-gray-500'}`}
                      >
                        等级 {villain.等级}
                      </span>
          ...
          {villains.length === 0 && <div className="text-center py-10 text-gray-600 text-xs">没有探测到目标</div>}
        </div>
        {/* Footer / Count */}
      ...
      <MetalContainer
        className="lg:col-span-2 h-full overflow-y-auto custom-scrollbar"
        glowColor="red"
        title="反派详细信息"
      >
        {selectedVillain ? (
          <div className="space-y-6">
            ...
            <div className="flex items-start gap-6 p-5 bg-white/5 rounded-xl border border-white/5 relative overflow-hidden group">
              ...
              <div className="relative w-24 h-24 shrink-0 hidden sm:block">
                <img
                  src={selectedVillain.avatarUrl}
                  alt={selectedVillain.姓名}
                  className="w-full h-full object-cover rounded-lg border-2 border-[#2c2e33]"
                />
              ...
              <div className="flex-1 space-y-3 relative z-10">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded bg-black border text-xs font-bold ${getThreatColor(selectedVillain.威胁度)} border-current opacity-90`}
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-white/10">
              ...
              <div>
                ...
                <ProgressBar
                  value={selectedVillain.善恶值}
                  min={-100}
                  max={100}
                  showValue={false}
                  biDirectional
                  color={selectedVillain.善恶值 >= 0 ? 'bg-blue-500' : 'bg-red-500'}
                />
