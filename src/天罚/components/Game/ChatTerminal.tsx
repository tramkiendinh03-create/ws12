        ...
        {messages.map(msg => {
          ...
          return (
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
              ...
              >
                {!isUser && !isSystem && (
                  <div className="text-[10px] text-gray-500 mb-1 font-bold tracking-wider">LILITH V. AKRA</div>
                )}
                {isSystem && (
                  <div className="text-[10px] text-yellow-500 mb-1 font-bold tracking-wider flex items-center gap-1">
                    <Terminal size={10} /> SYSTEM NOTIFICATION
                  </div>
                )}
