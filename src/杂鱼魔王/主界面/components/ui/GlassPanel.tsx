...
export const GlassPanel: React.FC<GlassPanelProps> = ({
  ...
  const baseStyles =
    'relative overflow-hidden backdrop-blur-xl border rounded-xl transition-all duration-300 flex flex-col';
  ...
  const variants = {
    ...
    crimson:
      'bg-gradient-to-br from-crimson-900/30 to-black/60 border-crimson-500/30 shadow-[0_0_20px_rgba(160,26,53,0.15)]',
    gold: 'bg-gradient-to-br from-gold-900/10 to-black/60 border-gold-500/20 shadow-[0_0_20px_rgba(255,215,0,0.1)]',
  };

  const hoverStyles = hoverEffect
    ? 'hover:border-white/20 hover:shadow-[0_0_25px_rgba(255,255,255,0.05)] hover:-translate-y-0.5'
    : '';
    ...
    <div className={`${baseStyles} ${variants[variant]} ${hoverStyles} ${className}`}>
      ...
      {(title || icon) && (
        <div className="relative flex shrink-0 items-center gap-3 border-b border-white/5 bg-black/20 p-4">
          ...
          {icon && (
            <span className={`relative z-10 ${variant === 'crimson' ? 'text-crimson-400' : 'text-gold-400'}`}>
              {icon}
            </span>
          )}
          {title && (
            <h3 className="font-display relative z-10 text-sm font-bold tracking-widest text-gray-200 uppercase drop-shadow-md">
              {title}
            </h3>
          )}
        </div>
      ...
      <div className="relative z-10 flex flex-1 flex-col overflow-hidden p-4">{children}</div>
    </div>
  );
};
