        ...
        <div className="custom-scrollbar flex-1 space-y-3 overflow-y-auto p-2 pr-1">
            {achievements.length === 0 ? (
                ...
                achievements.map((ach) => (
                    <div key={ach.id} className="to-void-900/60 border-gold-500/10 group relative rounded-lg border bg-gradient-to-br from-black/60 p-4 transition-all duration-300 hover:border-gold-500/40 hover:shadow-[0_0_15px_rgba(234,179,8,0.1)]">
                         ...
                         <div className="pl-2">
                             ...
                             <div className="ml-4 flex w-fit items-center gap-2 rounded border border-green-500/20 bg-green-950/30 px-3 py-1.5 text-[10px] text-green-300/90 shadow-inner">
                                <span className="text-green-500">❖</span> <span className="font-mono tracking-tight">{ach.reward}</span>
                             </div>
                         </div>
                    </div>
                ))
